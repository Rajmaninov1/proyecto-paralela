...
void FuncionPrincipal(int filas, int columnas, mapCell *A, mapCell *C) {
  ...
  if (taskid == MASTER)
  {
    ...
    averow = filas/numworkers;
    extrarow = filas%numworkers;
    avecolumn = columnas/numworkers;
    extracolumn = columnas%numworkers;
    offsetrows = 0;
    offsetcolumns = 0;
    mtype = FROM_MASTER;
    for (dest=1; dest<=numworkers; dest++){
      rows = (dest <= extrarow) ? averow+1 : averow;
      columns = (dest <= extracolumn) ? avecolumn+1 : avecolumn;
      // printf("Sending %d rows and columns %d to task %d offset=%d\n",filas,columnas,dest,offset);
      MPI_Send(&offsetrows, &offsetcolumns, 1, MPI_LONG, dest, mtype, MPI_COMM_WORLD);
      MPI_Send(&rows, &columns, MPI_LONG, 1, dest, mtype, MPI_COMM_WORLD);
      offsetrows = offsetrows + rows;
      offsetcolumns ? offsetcolumns + columns;
    }
    ...
    for (i=1; i<=numworkers; i++){
      source = i;
      MPI_Recv(&offsetrows, &offsetcolumns, 1, MPI_LONG, source, mtype, MPI_COMM_WORLD, &status);
      MPI_Recv(&rows, &columns, 1, MPI_LONG, source, mtype, MPI_COMM_WORLD, &status);
  ...
  {
    mtype = FROM_MASTER;
    MPI_Recv(&offsetrows, &offsetcolumns, 1, MPI_LONG, MASTER, mtype, MPI_COMM_WORLD, &status);
    MPI_Recv(&rows, &columns, 1, MPI_LONG, MASTER, mtype, MPI_COMM_WORLD, &status);
    for (i = offsetrows; i < filas - 1; i++) {
      for (j = offsetcolumns; j < columnas - 1; j++) {
        deltaT = 0.0;
        deltaQ = 0.0;
        deltaQ_rad = 0.0;
        deltaQ_flu = 0.0;
